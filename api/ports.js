 ...
 */
// FIX [BUG-PORT-INT]: env values were strings; cast to int so socket.connect() gets a number
const PORTS = process.env.PORTS_TO_CHECK
  ? process.env.PORTS_TO_CHECK.split(',').map(p => parseInt(p.trim(), 10)).filter(p => !isNaN(p) && p > 0 && p <= 65535)
